 ...
 * Author: mkalte666 (https://github.com/mkalte666)
*/
use std::hash::Hash;

use egui::{CornerRadius, CursorIcon, Id, Layout, Pos2, Rect, Sense, Ui, Vec2};
...
impl Splitter {
    ...
    pub fn show(self, ui: &mut Ui, add_contents: impl FnOnce(&mut Ui, &mut Ui)) {
        ...
        let mut ui_a = ui.new_child(
            egui::UiBuilder::new()
                .max_rect(child_rect_a)
        ...
        let mut ui_b = ui.new_child(
            egui::UiBuilder::new()
                .max_rect(child_rect_b)
                .layout(Layout::default()),
        );
