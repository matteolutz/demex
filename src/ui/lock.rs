...
pub fn locked_ui(ctx: &egui::Context, locked: &mut bool, lock_image: Option<&String>) {
        ...
        .show(ctx, |ui| {
            ui.vertical(|ui| {
                ...
                ui.centered_and_justified(|ui| {
                    if let Some(lock_image) = lock_image
                        .and_then(|lock_image| PathBuf::from(lock_image).canonicalize().ok())
                    {
                        ui.add(
